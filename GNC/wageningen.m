function [K_T, K_Q] = Wageningen(Ja,PD,AEAO,z)
% [K_T, K_Q] = Wageningen(Ja,PD,AEAO,z) computes the thrust and torque
...
K_T = sum(WagCThrust_stuv.*((Ja).^WagThrust_s).*(PD.^WagThrust_t).*...
    (AEAO.^WagThrust_u).*(z.^WagThrust_v));
K_Q = sum(WagCTorque_stuv.*((Ja).^WagTorque_s).*(PD.^WagTorque_t).*...
