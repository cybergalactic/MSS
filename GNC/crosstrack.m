...
%
% Outputs:  
%   (x_proj, y_proj)    : Origin of path-tangential frame expressed in NED
%   y_e                 : Cross-track error
%
% Usage:
%   [x_proj, y_proj, ye] = crosstrack(150, 300, 0, 0, 80, 30, 1)  % Plot geometry
%   [x_proj, y_proj, ye] = crosstrack(150, 300, 0, 0, 80, 30)     % No plot
%
...
if (nargin == 7 && flag == 1)
    ...
    plot(y_proj,x_proj,'ks','MarkerFaceColor','c','MarkerSize',12,'LineWidth',2)
    plot([y_ref y_t],[x_ref x_t],'k-','LineWidth',2)
    plot([y_proj y],[x_proj x],'r-.','LineWidth',2)
    hold off
    grid;
    xlabel('East [m]')
    ylabel('North [m]')
