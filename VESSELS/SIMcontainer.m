echo on
% SIMcontainer User editable script for simulation of the container ship 
% % under feedback control. Both the linear model Lcontainer.m and 
% nonlinear model container.m are simulated.
%
...
for i=1:N+1
    ...
    % ship model
    [xdot,U] = container(x,[delta_c n_c]);          % ship models
...
figure(2)
subplot(221),plot(t,r,'r',t,r2,'b'),xlabel('time (s)'),title('yaw rate r (deg/s)'),grid
legend('nonlinear model','linear model')
subplot(222),plot(t,phi,'r',t,phi2,'b'),xlabel('time (s)'),title('roll angle \phi (deg/s)'),grid
legend('nonlinear model','linear model')
subplot(223),plot(t,psi,'r',t,psi2,'b'),xlabel('time (s)'),title('yaw angle \psi (deg)'),grid
legend('nonlinear model','linear model')
subplot(224),plot(t,delta,'r',t,delta2,'b'),xlabel('time (s)'),title('rudder angle \delta (deg)'),grid
legend('nonlinear model','linear model')
