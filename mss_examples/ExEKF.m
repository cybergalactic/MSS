...
N  = 1000;		  % no. of iterations (sampling time: h  = 1/f_samp) 
...
for i=1:N+1
   ...
   % Discrete-time matrices: PHI and GAMMA
   PHI   = [ 1   h
   ...
   % KF gain      
   K = X_prd * H' * inv( H * X_prd * H' + R );
        
   % corrector   
   IKH = eye(2) - K*H;
   X_hat = IKH * X_prd * IKH' + K * R * K';
   eps = z - H * x_prd;
   x_hat = x_prd + K * eps;   
   
   % store simulation data in a table   
   simdata(i,:) = [t x' x_hat' X_hat(1,1) X_hat(2,2) ];    
   ...
   % Predictor (k+1)  
   x_prd = f_k;
   X_prd = PHI * X_hat * PHI' + GAMMA * Q * GAMMA';
